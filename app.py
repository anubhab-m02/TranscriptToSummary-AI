...
def generate_summary(transcript, length, key_takeaways, language, user_interests):
    interests_prompt = f" Focus on aspects related to {user_interests}." if user_interests else ""
    prompt = f"Please provide a {length} summary of the following lecture transcript and provide {key_takeaways} key takeaways in {language}.{interests_prompt}\n\n{transcript}"
    return generate_content(prompt)

def generate_questions(summary, num_questions, difficulty, language):
    prompt = f"Based on the following summary, generate {num_questions} {difficulty}-level questions in {language} without providing answers:\n\n{summary}"
    return generate_content(prompt)
    ...
    if st.session_state.transcript:
        ...
        if st.button("🚀 Generate Summary", key="generate_summary_button"):
            with st.spinner("Generating summary..."):
                simulate_progress()
                st.session_state.summary = generate_summary(st.session_state.transcript, summary_length, key_takeaways, language, user_interests)
            ...
            with st.expander("Generate Questions", expanded=True):
                ...
                if st.button("🧠 Generate Questions", key="generate_questions_button", use_container_width=True):
                    with st.spinner("Generating questions..."):
                        st.session_state.questions = generate_questions(st.session_state.summary, num_questions, difficulty, language)
